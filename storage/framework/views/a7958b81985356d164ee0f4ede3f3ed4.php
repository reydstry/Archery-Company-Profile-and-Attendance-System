<?php $__env->startSection('title', 'Slot & Coach Assignment'); ?>
<?php $__env->startSection('subtitle', 'Kelola slot per sesi dan assignment coach'); ?>
...
<div class="w-full px-4 sm:px-6 lg:px-8 space-y-4"
    ...
    <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        <div class="flex items-center justify-between gap-3 px-5 py-3.5 bg-[#1a307b] border-b border-slate-100">
            <div class="flex items-center gap-2.5">
                ...
                <h3 class="text-sm font-semibold text-white">Pilih Sesi Latihan</h3>
            </div>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-3 p-4 gap-3">
            <div class="md:col-span-2">
                <select x-model="selectedSession" @change="loadSessionDetail()" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#1a307b]/30">
                    <template x-for="session in sessions" :key="session.id">
                        <option :value="session.id" x-text="formatDate(session.date)"></option>
                    </template>
                </select>
            </div>
    ...
    <div class="hidden lg:block bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden ">
        ...
        <div class="grid grid-cols-12 bg-slate-50/80 border-b border-slate-200 px-5 py-2.5">
            ...
            <div class="col-span-2 text-xs font-bold text-slate-800 uppercase tracking-wider text-center">Waktu</div>
            <div class="col-span-2 text-xs font-bold text-slate-800 uppercase tracking-wider text-center">Slot</div>
            <div class="col-span-2 text-xs font-bold text-slate-800 uppercase tracking-wider text-center">Pelatih</div>
            <div class="col-span-2 text-xs font-bold text-slate-800 uppercase tracking-wider text-center">Status</div>
            <div class="col-span-3 text-xs font-bold text-slate-800 uppercase tracking-wider pl-[100px]">Aksi</div>
        </div>
        ...
        <template x-if="!isLoading && filteredSlots.length === 0">
            <div class="bg-white rounded-2xl border border-slate-200 p-10 text-center">
                ...
                <p class="text-slate-500 font-semibold text-sm mb-3">Belum ada slot</p>
            </div>
        </template>

        <template x-for="(slot, index) in filteredSlots" :key="slot.id">
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\Project\club-panahan\resources\views\dashboards\admin\training\slots-coach-assignment.blade.php ENDPATH**/ ?>